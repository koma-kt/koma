...
3.1.0 から 4.0.0-beta01 までで、Middleware → Plugin の置換、Store ライフサイクル制御の policy 化、`koma-test` 追加、Compose 連携の見直しなど、利用者が触れる API が広範囲に変わった。
...
- `Middleware<S, A, E>` / `MiddlewareScope<A>` 削除。`Plugin<S, A, E>` / `PluginScope<S, A>` に置換。
  - 関連: `koma-core/src/commonMain/kotlin/koma/core/Plugin.kt`、`PluginScope.kt`
...
- `koma-logging`: `LoggingMiddleware` 抽象クラス削除。`simpleLogging(...)` は残るが戻り値は `Plugin`。
- `koma-message`: `MessageMiddleware.kt` 削除。`receiveMessages(...)` は残るが戻り値は `Plugin`、ラムダレシーバが `MiddlewareScope<A>` → `PluginLaunchScope<S, A>`。
...
- 新規 opt-in アノテーション `@InternalKomaApi` を追加。3.1.0 で参照できていた一部 API が `internal` 化／要 opt-in 化されている可能性がある。

## B. 4.0 で追加された新機能

### B-1. 新モジュール `koma-test`

公開 API は 2 ファイル: `koma-test/src/commonMain/kotlin/koma/test/StoreExtensions.kt`、`StoreRecorder.kt`。
...
中核: `koma-core/src/commonMain/kotlin/koma/core/Plugin.kt`、`PluginScope.kt`。
...
- `fun start()`: 明示的に起動を要求（即 return、完了は待たない。完了まで待ちたいときは `koma-test` の `startAndWait()`）。
...
| 影響度 | 主な対象 |
|---|---|
| 必ず触る | Middleware → Plugin（`koma-logging` / `koma-message` 含む）／`Store(block)` オーバーロード／`ExceptionHandler.Default`／`rememberViewStore(store, autoDispose=…)` |
...
| 4.0 で得られる新機能 | `koma-test`（recorder / patch / *AndWait）／`Plugin` 4 フック ＋ `PluginScope.launch`／`AutoStartPolicy` / `PendingActionPolicy` / `PluginExecutionPolicy`／`ActionScope.launch` ＋ `LaunchControl` / `LaunchLane` / `cancelLaunch`／全スコープの `clearPendingActions()`／`Store.start()` / `close()` / `AutoCloseable`／`Store(initialState, context) { }` |
